...
%    el modo TANGENCIAL (lento, ~1120 m/s, energía por el contorno).
%    Con angle=90 la fuerza apunta horizontalmente hacia el centro del
%    disco → excita el modo RADIAL (rápido, ~1525 m/s, cruza el diámetro).
...
rho = 661; c11 = 1.537e9; c33 = 8.29e8; c55 = 1.81e8; c13 = 4.289e8;
...
fprintf('  V(θ=0°)  = %.1f m/s  |  tesis: 1525 m/s  |  error: %.1f%%\n', ...
    V0,  100*abs(V0-1525)/1525);
fprintf('  V(θ=90°) = %.1f m/s  |  tesis: 1120 m/s  |  error: %.1f%%\n', ...
    V90, 100*abs(V90-1120)/1120);
fprintf('  TOF analítico (0.30m): %.2f µs\n', TOF_ref);
fprintf('  Rango experimental tesis: 194 %s\n', ...
...
file_circ_sano  = 'Mesh_externa/Pino_C_sin_hueco_N_2/OUTPUT_FILES/AA.S0001.BXX.semd';
file_circ_hueco = 'Mesh_externa/Pino_C_con_hueco_N_3/OUTPUT_FILES/AA.S0001.BXX.semd';

% Malla interna de referencia (validada)
file_int_sano   = 'Mesh_interna/Pino_sin_hueco_Norm/OUTPUT_FILES/AA.S0001.BXX.semd';
file_int_hueco  = 'Mesh_interna/Pino_con_hueco_N_C/OUTPUT_FILES/AA.S0001.BXX.semd';
...
if tof_env(1)>0
    ...
    in_range = tof_env(1)>=180 && tof_env(1)<=200;
    if in_range
        fprintf('    Rango exp. tesis Fig.61 (~194µs): DENTRO ✓\n');
    else
        fprintf('    Rango exp. tesis Fig.61 (~194µs):\n');
        fprintf('      TOF_sim=%.2fµs vs mínimo exp=158µs (dif=%.2fµs=%.1f%%)\n', ...
            tof_env(1), abs(tof_env(1)-194), 100*abs(tof_env(1)-194)/194);
        fprintf('      Diferencia explicable: error velocidad %.1f%% (<5%%) ✓\n', err_V);
        fprintf('      TOF analítico (194µs) coincide con el mínimo experimental.\n');
