...
V0  = sqrt(c11/rho);    % radial 1897 m/s
V90 = sqrt(c33/rho);    % tangencial 1383 m/s
dist = 0.30;
TOF_ref = dist/V0*1e6;
DT = 2.0e-7;
...
fprintf('  Madera roble (radial): V=%.0f m/s\n', V0);
...
for i=1:nA
    ...
    sn_energia = s / max(abs(datos{1}.s));
    energia(i) = trapz(t, sn_energia.^2);
...
set(gca,'XTickLabel',{'Sano','H.Aire','H.Agua'});
