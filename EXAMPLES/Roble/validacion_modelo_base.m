...
%  Tronco de roble 30x30 cm, malla interna estructurada
%
...
% Material: roble anisótropo (Tabla 2, pág.27 tesis)
rho_oak = 706.0;       % kg/m³
c11_oak = 2.54e9;      % Pa
c13_oak = 7.5832e8;    % Pa
c33_oak = 1.35e9;      % Pa
...
% Constantes elásticas roble (Par_file, Tabla 3 tesis pág.41)
rho = 706;        % kg/m³
c11 = 2.54e9;     % Pa  (≡ ER)
c33 = 1.35e9;     % Pa  (≡ ET)
c55 = 3.19e8;     % Pa  (≡ GRT)
...
fprintf('  θ=0°  (radial):     %.1f m/s  |  Tesis: 1898 m/s  |  error: %.1f%%\n', ...
    V0,  100*abs(V0-1898)/1898);
fprintf('  θ=90° (tangencial): %.1f m/s  |  Tesis: 1385 m/s  |  error: %.1f%%\n', ...
...
fprintf('  Rango tesis (Ash Ø30cm, dir. radial, Fig.61): 158-190 µs %s\n', ...
    ternario(TOF_ref>=158 && TOF_ref<=190,'✓','⚠'));
...
tab = {
    'V(θ=0°) [m/s]',            sprintf('%.1f',V0),         '1898',              ternario(abs(V0-1898)/1898<0.02,'✓','✗');
    'V(θ=90°) [m/s]',           sprintf('%.1f',V90),        '1385',              ternario(abs(V90-1385)/1385<0.02,'✓','✗');
    'TOF sano xcorr [µs]',      sprintf('%.1f',TOF_xcorr_s),'158.2 (ref)',       ternario(abs(TOF_xcorr_s-TOF_ref)/TOF_ref<0.15,'~','✗');
    'TOF sano threshold [µs]',  sprintf('%.1f',TOF_th_s),   '158.2 (ref)',       ternario(abs(TOF_th_s-TOF_ref)/TOF_ref<0.10,'✓','✗');
    'TOF sano envolvente [µs]', sprintf('%.1f',TOF_env_s),  '158.2 (ref)',       ternario(abs(TOF_env_s-TOF_ref)/TOF_ref<0.02,'✓','~');
...
% Etiqueta del tronco
text(tronco.xmin+0.01, tronco.zmax-0.02, 'Roble anisótropo', ...
...
title({'Sismogramas SPECFEM2D — Roble (malla interna)', ...
